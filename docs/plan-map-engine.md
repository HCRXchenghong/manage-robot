...
### 阶段 2 浏览器编辑地图
- 2.1 多边形套索（2D 视图点选、双击闭合）→ 拉伸体内点删除（存差异 bitmap）/分类
- 2.2 撤销/重做；「保存为新版本」POST /api/maps（fleet-hub + PG：版本号、作者、时间戳、差异）；版本列表可恢复/对比
- 2.3 矢量区域：禁行/缓行多边形绘制，2D/3D 叠加渲染，与接管页 fencing 提示联动
- 验收：真实地图圈删一块并保存 → 重载仍生效 → 一键恢复原版本；区域大屏可见、接管 fencing 提示包含
