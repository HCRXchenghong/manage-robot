// 组织与成员管理（精简版）：页面只留成员列表，配置全部收进弹窗。
//   超管视角：新建账号 / 分组管理 / 成员管理 都弹窗操作。
...
const emptyForm = { username: "", display_name: "", password: "", role: "user", groups: [] as string[], phone: "" };
...
export default function AdminPage({ me }: { me: Me }) {
  ...
  const mu = users.find((u) => u.username === manage) || null;
  ...
  const toggleIn = (gid: string, list: string[]) =>
    list.includes(gid) ? list.filter((x) => x !== gid) : [...list, gid];
    ...
    <div style={{ padding: 16, overflow: "auto", height: "100%" }}>
      <div className="row" style={{ justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
        ...
        <div className="btn-row">
          <button className="btn primary" onClick={() => { setForm(emptyForm); setCreateOpen(true); }}>新建账号</button>
          {isSuper && <button className="btn" onClick={() => setGroupsOpen(true)}>分组管理</button>}
        </div>
      </div>
                ...
                <td>
                  {u.username !== me.username && u.role !== "super" && (
                    <span className="row" style={{ gap: 4 }}>
                      <button className="btn small" onClick={() => { setEditPwd(""); setManage(u.username); }}>管理</button>
                      <button
                        className="btn small danger"
                        onClick={() => {
                          if (window.confirm("删除账号 " + u.username + "？该操作不可恢复。")) void wrap(() => deleteUser(u.username), "已删除 " + u.username);
                        }}
                      >删除</button>
                    </span>
                  )}
      ...
      {createOpen && (
        <Modal title="新建账号" onClose={() => setCreateOpen(false)} width="min(640px, 92vw)">
          <div className="row" style={{ gap: 8, flexWrap: "wrap", marginBottom: 10 }}>
            ...
            <select className="input" style={{ width: 130 }} value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value })}>
          ...
          <div className="row" style={{ gap: 8, flexWrap: "wrap", marginBottom: 10 }}>
            <input className="input" style={{ width: 200 }} type="password" placeholder="初始密码（≥8位含3类字符）" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} />
            <input className="input" style={{ width: 170 }} placeholder="手机号（选填，用于登录）" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value.replace(/\D/g, "") })} />
          </div>
            ...
            <div className="row" style={{ gap: 6, flexWrap: "wrap", marginBottom: 10 }}>
              ...
              {groups.map((g) => (
                <label key={g.id} className="chip" style={{ cursor: "pointer" }}>
                  <input type="checkbox" checked={form.groups.includes(g.id)} onChange={() => setForm({ ...form, groups: toggleIn(g.id, form.groups) })} />
                  {" "}{g.name}
                </label>
              ))}
            </div>
          )}
          {!isSuper && <div className="muted" style={{ fontSize: 12, marginBottom: 10 }}>新账号创建后即可登录本平台。</div>}
          <div className="btn-row" style={{ justifyContent: "flex-end" }}>
            <button className="btn" onClick={() => setCreateOpen(false)}>取消</button>
            <button
              className="btn primary"
              onClick={() => void wrap(async () => { await createUser(form); setCreateOpen(false); }, "账号已创建")}
            >创建账号</button>
          </div>
        </Modal>
      )}
      ...
      {groupsOpen && isSuper && (
        <Modal title="分组管理" onClose={() => setGroupsOpen(false)} width="min(820px, 94vw)">
          <div className="btn-row" style={{ marginBottom: 12 }}>
            ...
            <button className="btn primary" onClick={() => void wrap(async () => { await createGroup(newGroup); setNewGroup(""); }, "分组已创建")}>创建分组</button>
          </div>
          <table className="table">
            <thead>
              <tr>
                ...
                <tr key={g.id}>
                  ...
                  <td>
                    <button
                      className="btn small danger"
                      onClick={() => {
                        if (window.confirm("删除分组 " + g.name + "？组内成员将失去该分组权限。")) void wrap(() => deleteGroup(g.id), "分组已删除");
                      }}
                    >删除</button>
                  </td>
                </tr>
      ...
      {mu && (
        <Modal title={"成员管理 · " + mu.username} onClose={() => setManage("")} width="min(520px, 92vw)">
          <div className="muted" style={{ marginBottom: 12 }}>
            {mu.display_name} · {roleLabel(mu.role)}
          </div>
          {isSuper && (
            <div className="row" style={{ gap: 6, flexWrap: "wrap", marginBottom: 14 }}>
              ...
              {groups.map((g) => (
                <label key={g.id} className="chip" style={{ cursor: "pointer" }}>
                  <input
                    type="checkbox"
                    checked={(mu.groups || []).includes(g.id)}
                    onChange={() => void wrap(() => setUserGroups(mu.username, toggleIn(g.id, mu.groups || [])), "分组已更新")}
                  />
                  {" "}{g.name}
                </label>
              ))}
          ...
          <div className="row" style={{ gap: 6 }}>
            ...
            <button className="btn small" onClick={() => void wrap(() => resetUserPassword(mu.username, editPwd), "密码已重置，该用户会话已强制下线")}>
              重置密码
            </button>
          </div>
        </Modal>
