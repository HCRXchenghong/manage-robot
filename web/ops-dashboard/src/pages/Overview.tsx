...
export default function Overview({ fleet, selected, onSelect }: Props) {
  ...
  const leftOverlay = (
    <SideCol id="left" side="left" label="车辆列表 / 终端 / 告警">
      ...
      <div className="ov-split">
        <div className="ov-click" onDoubleClick={openIfPlain(() => setBigTerminal(true))}>
          <TerminalPanel vehicle={selected} fixed onExpand={() => setBigTerminal(true)} />
        </div>
        <div className="ov-click" onDoubleClick={openIfPlain(() => setBigEvents(true))}>
  ...
  const rightOverlay = (
    <SideCol id="right" side="right" label="详情 / 视频" open={rightOpen} onOpenChange={setRightOpen}>
      <div className="ov-click ov-fit" onDoubleClick={openIfPlain(() => setBigDetail(true))}>
        <VehicleDetail fleet={fleet} vehicle={selected} onSelect={onSelect} compact onExpand={() => setBigDetail(true)} />
      </div>
      <div className="ov-click" onDoubleClick={openIfPlain(() => setBigVideo(true))}>
    ...
    <div className="overview">
      <div className="overview-toolbar">
        ...
        <span className="muted" style={{ fontSize: 11 }}>
          数据源：{fleet.source === "live" ? "fleet-hub 实时" : fleet.source === "degraded" ? "最后验证快照（链路恢复中）" : "尚未取得车队快照"} · 1Hz 推送
        </span>
      </div>
      ...
      {bigTerminal && (
        <Modal
          title={"远程终端" + (selected ? " · " + selected.vehicle_id : "")}
